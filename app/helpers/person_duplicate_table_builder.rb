...
class PersonDuplicateTableBuilder
  ...
  def initialize(entries, group, template)
    ...
    @entries = entries
    @group = group
    @cols = [:first_name, :last_name, :nickname, :company_name, :birth_year, :town, :roles_list, :actions]
  end
  ...
  def action_col(entry, pnr)
    content_tag(:td, class: 'right vertical-middle', rowspan: 2) do
      content = ''
      if can?(:merge, entry)
        content += action_button(t('merge.action'), new_merge_path(entry), :'user-friends', remote: true)
      end
      if can?(:acknowledge, entry)
        content += action_button(t('acknowledge.action'), new_acknowledge_path(entry), :'user-slash', remote: true)
      end
      content.html_safe
    end
  end
